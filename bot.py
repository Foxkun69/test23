...
raj = commands.Bot(description='SELFBOT CREATED BY Foxkun',
...
async def on_ready():
      print(
        Center.XCenter(
            ...
            f"""[=]-------------------------------------------------------------------------------------------[=]
[  SELFCORD  VERSION  2  -  MADED  BY  :-  fox_kun.  -  LOGINED  AS  :-  {raj.user.name}  ]
[=]-------------------------------------------------------------------------------------------[=]
""",
                1,
            )
...
class AFK(commands.Cog):
    ...
    async def afk(self, ctx, *, reason="busy so don't ping"):
        ...
        await ctx.send(f"<a:arrow:1274613870631194710> **You are now AFK..**")
        self.save_afk_data()
    ...
    async def unafk(self, ctx):
        ...
        if user_id in self.afk_data:
            ...
            await ctx.send(f"<a:arrow:1274613870631194710> **You are no longer AFK**")
            self.save_afk_data()
        else:
            await ctx.send(f"{ctx.author.mention}, <a:arrow:1274613870631194710> **you are not AFK**")
...
@raj.command()
async def help(ctx):
    message = '''<a:thekings:1293113386439147582> ** S E L F B O T - V E R S I O N - 2 ** <a:thekings:1293113386439147582>
...
<a:sl:1310589238320042084> **Afk** :- `.afk <reason>`
<a:sl:1310589238320042084> **Remove Afk** :- `.unafk`
<:badge:1310556291743350845> **Status Rotator** :- `.rotate <emoji id , msg> / <emoji id , msg> / <repeat again>`
<:badge:1310556291743350845> **Stop Rotator** :- `.stop_rotate`
<:badge:1310556291743350845> **Create Status** :- `.stream/play/watch/listen <title>`
<:badge:1310556291743350845> **Remove Status** :- `.stopactivity`
🍌 **Show Allcmds** :- `.help`
...
<a:crt:1303610340415766590> **Upi Id** :- `.upi`
<a:crt:1303610340415766590> **Qr Code** :- `.qr`
<a:crt:1303610340415766590> **Custom Qr** :- `.cqr <amt> <note>`
🍗 **Send Ltc** :- `.send <addy> <amount>`
🍗 **Check Balance** :- `.bal <addy>`
🍗 **Check Mybal** :- `.mybal`
🍗 **Ltc Addy** :- `.addy`
🍗 **Crypto Price In Usd** :- `.ltc/sol/btc/usdt`
<:6020:1310569690178916352> **Calculate** :- `.math <equation>`
<:6020:1310569690178916352> **Inr To Crypto** :- `.i2c <inr amount>`
<:6020:1310569690178916352> **Crypto To Inr** :- `.c2i <crypto amount>`
<:6020:1310569690178916352> **Ltc To Usd** :- `.l2u <ltc amount>`
<:6020:1310569690178916352> **Usd To Ltc** :- `.u2l <usd amount>`
<a:nitro:1310556241122164766> **Check Promo** :- `.checkpromo <promo>`
<a:nitro:1310556241122164766> **Check Token** :- `.checktoken <token>`
...
<:owo:1310556244532265001> **Owo Grind Start** :- `.owostart`
<:owo:1310556244532265001> **Owo Grind Stop** :- `.owostop`
<:owo:1310556244532265001> **Abuse** :- `.abuse <user>`
<:owo:1310556244532265001> **Save Transcript** :- `.savetrs`
<:owo:1310556244532265001> **Mass React** :- `.massreact <emote>`
🧿 **Hide** :- `.hide`
🧿 **Unhide** :- `.unhide`
🧿 **Restart Bot** :- `.restart`
🧿 **Nuke Server** :- `.nukesrv`
...
@raj.command()
async def upi(ctx):
    message = (f"<a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590> **UPI** <a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590> <a:crt:1303610340415766590>")
    message2 = (f"{Upi}")
...
async def qr(ctx):
    message = (f"{Qr}")
    message2 = (f"**chuyển xong thì chụp bill** ")
    await ctx.send(message)
    await ctx.send(message2)
...
async def math(ctx, *, equation):
    ...
    if response.status_code == 200:
        ...
        await ctx.send(f'<a:arrow:1274613870631194710> **EQUATION**: `{equation}`\n\n<a:arrow:1274613870631194710> **Result**: `{result}`')
        await ctx.message.delete()
    else:
        await ctx.reply('<a:arrow:1274613870631194710> **Failed**')
...
async def i2c(ctx, amount: str):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **EQUATION**: `{amount}/{I2C_Rate}`\n\n<a:arrow:1274613870631194710> **Result** : `${inr_amount:.2f}`")
    await ctx.message.delete()
...
async def c2i(ctx, amount: str):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **EQUATION**: `{amount}*{C2I_Rate}`\n\n<a:arrow:1274613870631194710> **Result** : `₹{usd_amount:.2f}`")
    await ctx.message.delete()
...
async def stream(ctx, *, message):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **Stream Created** : `{message}`")
...
async def play(ctx, *, message):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **Status For PLAYZ Created** : `{message}`")
...
async def watching(ctx, *, message):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **Watching Created**: `{message}`")
...
async def listening(ctx, *, message):
    ...
    await ctx.reply(f"<a:arrow:1274613870631194710> **Listening Created**: `{message}`")
...
async def exch(ctx, *, text):
    await ctx.message.delete()
    main = text
    await ctx.send(f'+vouch {User_Id} <số lượng> <sản phẩm> <giá tiền> | {main} ')
...
async def vouch(ctx, *, text):
    await ctx.message.delete()
    main = text
    await ctx.send(f'+vouch @{User_Id} <quantity> <product> <value> | {main} ')
    await ctx.send(f'{SERVER_Link}')
    await ctx.send(f'**pls vouch me here**')
...
async def ltc(ctx):
    ...
    if response.status_code == 200:
        ...
        await ctx.send(f"<a:arrow:1274613870631194710> **The Price Of Ltc Is :** `{price:.2f}`")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} LTC PRICE SENT✅ ")
    else:
        await ctx.send("**<a:arrow:1274613870631194710> Failed To Fetch**")
...
async def sol(ctx):
    ...
    if response.status_code == 200:
        data = response.json()
        price = data['market_data']['current_price']['usd']
        await ctx.send(f"<a:arrow:1274613870631194710> **The Price Of Sol Is :** `{price:.2f}`")
...
async def usdt(ctx):
    ...
    if response.status_code == 200:
        data = response.json()
        price = data['market_data']['current_price']['usd']
        await ctx.send(f"<a:arrow:1274613870631194710> **The Price Of Usdt Is :** `{price:.2f}`")
...
async def btc(ctx):
    ...
    if response.status_code == 200:
        data = response.json()
        price = data['market_data']['current_price']['usd']
        await ctx.send(f"<a:arrow:1274613870631194710> **The Price Of Btc Is :** `{price:.2f}`")
...
async def ar(ctx, *, trigger_and_response: str):
    ...
    await ctx.send(f'<a:arrow:1274613870631194710> **Auto Response Has Added.. !** **{trigger}** - **{response}**')
    await ctx.message.delete()
...
async def removear(ctx, trigger: str):
    ...
    if trigger in data:
        ...
        await ctx.send(f'<a:arrow:1274613870631194710> **Auto Response Has Removed** **{trigger}**')
        await ctx.message.delete()
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} AUTO RESPOND REMOVE✅ ")
    else:
        await ctx.send(f'<a:arrow:1274613870631194710> **Auto Response Not Found** **{trigger}**')
...
async def csrv(ctx, source_guild_id: int, target_guild_id: int):
    ...
    if not source_guild or not target_guild:
        await ctx.send("<a:arrow:1274613870631194710> **Guild Not Found**")
        return
...
async def send(ctx, addy, value):
    try:
        ...
        message = await ctx.send(f"<a:arrow:1274613870631194710> **Sending {value}$ To :-** {addy}")
        ...
        await message.edit(content=f"<a:arrow:1274613870631194710> **Successfully Sent {value}$ To {addy}**\nhttps://live.blockcypher.com/ltc/tx/{response_data["txId"]}")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}LTC SEND SUCCESS✅ ")
    except:
        await ctx.send(content=f"<a:arrow:1274613870631194710> **Failed to send LTC Because** :- {response_data["cause"]}")
...
async def clear(ctx, times: int):
    ...
    await ctx.send(f"<a:arrow:1274613870631194710> **Deleted {times} Messages**")
...
async def am(ctx, time_in_sec: int, channel_id: int, *, content: str):
    ...
    if channel is None:
        await ctx.send("<a:arrow:1274613870631194710> `Channel not found.`")
        return

    if time_in_sec <= 0:
        await ctx.send("<a:arrow:1274613870631194710> `Time must be greater than 0.`")
        return

    auto_messages = load_auto_messages()

    if str(channel_id) in auto_messages:
        await ctx.send(f"<a:arrow:1274613870631194710> **Auto Message already exists for channel {channel_id}.**")
        return
...
async def am_stop(ctx, channel_id: int):
    ...
    if channel_id in tasks_dict:
        ...
        await ctx.send(f"<a:arrow:1274613870631194710> **Auto Message Stopped for channel {channel_id}.**")
        print("Automessage Stoped Succesfully")
    else:
        await ctx.send("<a:arrow:1274613870631194710> `No auto message task found for this channel.`")
...
async def joke(ctx):
    ...
    joke = response.json()
    await ctx.send(f"<a:arrow:1274613870631194710> {joke['setup']} - {joke['punchline']}")
    await ctx.message.delete()
...
async def l2u(ctx, ltc_amt: float):
    ...
    try:
        ...
        await ctx.send(f"<a:arrow:1274613870631194710> **EQUATION**: `{ltc_amt}*{ltc_to_usd_rate}`\n\n<a:arrow:1274613870631194710> `{ltc_amt} LTC = {output} USD`")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} L2U✅ ")
    except requests.RequestException as e:
        await ctx.send(f"<a:arrow:1274613870631194710> `Error fetching Litecoin price: {e}`")
...
async def u2l(ctx, usd_amt: float):
    ...
    try:
        ...
        await ctx.send(f"<a:arrow:1274613870631194710> **EQUATION**: `{usd_amt}/{ltc_to_usd_rate}`\n\n<a:arrow:1274613870631194710> `{usd_amt} USD = {output} LTC`")
...
- VERSION > 2
- DEVELOPER > `fox_kun.`
- SUPPORT SERVER > https://discord.gg/KxJgsC2wFc
- AUTOBUY LINK > https://foxstorevn.mysellauth.com/''')
    await ctx.message.delete()
...
async def checktoken(ctx , tooken):
    ...
    if r.status_code == 200:
        ...
        await ctx.send("<a:arrow:1274613870631194710> Invalid Token or Locked or flagged")
...
async def translate(ctx, *, text: str):
    ...
    try:
        ...
        await ctx.send("<a:arrow:1274613870631194710> **Error**: Could not translate text. Please try again later.")
...
async def avatar(ctx, user: discord.User):
    ...
    try:
        ...
        await ctx.send("<a:arrow:1274613870631194710> User Don't Have Avatar")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} AVATAR✅ ")
        
@raj.command()
async def icon(ctx):
    await ctx.message.delete()
    server_icon_url = ctx.guild.icon_url if ctx.guild.icon else "<a:arrow:1274613870631194710> No server icon"
...
async def nukesrv(ctx):
    ...
    message_text = '# FUCKED BY Fox  :   ||@everyone||'
